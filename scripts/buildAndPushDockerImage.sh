...
if [ "$branchName" == "master" ]; then
	...
	if [[ $gitLongTag =~ [0-9]+.[0-9]+.[0-9]+-0-[0-9a-g]{8}$ ]]; then 
		...
		echo "Invalid tag on the master branch.  Make sure you have just tagged the master branch with something like '1.2.3'"
		exit -1	
	fi

else
	# in other branches then master, the imageTag is the branchName
	imageTag=$branchName
...
if [ "$branchName" == "master" ]; then
	docker tag $imageName:$imageTag $imageName:toto
	docker push $imageName:toto
fi
